 ...
 * describes address `0x7' as "Date of month" instead of "Day of month." I am
...
/*
 * Three divider-selection bits, which select the time-base frequency.
 * I am just repeating the manual, I do not know much about this.
 */ 
#define CMOS_RTC_DVX 0x70

/*
 * Four divider-selection bits, which select a divider output frequency.
 * I am just repeating the manual, I do not know much about this.
 */ 
#define CMOS_RTC_RTX 0xf
...
/*
 * 0 - Updates RTC once per second as normal. Disables the "update mode" enabled
 * by setting this bit.
 * 1 - Stops updating, so that the RTC can be written to by the CPU (us).
*/ 
#define CMOS_RTC_SET 0x80

/*
 * Periodic Interrupt Enable bit. Enables an interrupt fired at a rate specified
 * by the `CMOS_RTC_RTX' and `CMOS_RTC_DVX' bits.
 * 0 - Disables the interrupt (default).
...
/*
 * Square Wave Frequency Enable bit. Enables a square wave frequency specified
 * by the `CMOS_RTC_RTX' and `CMOS_RTC_DVX' bits.
 * 0 - Disables the frequency (default). 
 * 1 - Enables the frequency.
 */
...
/*
 * The following four bits are read-only and work in conjunction with the
 * interrupts that can be issued through CMOS configuration.
 */

/* An interrupt has been fired. */
#define CMOS_RTC_IRQF 0x80

/* A periodic interrupt has been fired. */
#define CMOS_RTC_PF 0x40

/* An alarm interrupt has been fired. */
#define CMOS_RTC_AF 0x20

/* An update-ended interrupt has been fired. */
#define CMOS_RTC_UF 0x10

/* The rest of the flag space is reserved. */
...
/*
 * Checksum Status Indicator.
 * 0 - Checksum good. 
 * 1 - Checksum bad.
 */ 
#define CMOS_CSI 0x40
...
#define CMOS_SHUT_NORMAL 0x0
#define CMOS_SHUT_MEM_SIZE 0x1
#define CMOS_SHUT_MEM_TEST 0x2
#define CMOS_SHUT_MEM_ERR 0x3
#define CMOS_SHUT_BOOT_REQ 0x4
#define CMOS_SHUT_JMP_REQ_INT 0x5
#define CMOS_SHUT_PM7_PASS 0x6
#define CMOS_SHUT_PM7_FAIL 0x7
#define CMOS_SHUT_PM1_FAIL 0x8
#define CMOS_SHUT_BLK_MV_REQ 0x9
#define CMOS_SHUT_JMP_REQ 0xa
 ...
 * on "no."
 *
 * I hope I have not made a mistake here... it is too easy to get confused with
